...
def addBubble(data, row, col, color, feature, Bubbletype) :
    x, y = Bubble.getXY(data, row, col)
    newBubble = Bubble(row, col, x, y, color, feature, Bubbletype)
...
def processPostCol(data, color) :
    shooter = data.tempBubbles[0]
    data.canShoot = True
    feature = shooter.feature
    Bubbletype = shooter.type
    addBubble(data, data.destRow, data.destCol, color, feature, Bubbletype)
...
# enemy used item 'addLine'
def attackAddLine(data) :
...
def applyOtherItem(data, item) : 
    data.itemTimer = data.itemCountdown
    if item == "addBubble" : attackAddBubble(data)
...
def drawGuideLine(canvas, data) :
    x = data.anchorX
    y = data.anchorY
    anchorTop = data.anchorLen
    guide = data.guideLen
    x0 = x - data.guideLen * math.cos(data.anchor)
    y0 = y - data.guideLen * math.sin(data.anchor)
    x1 = x - anchorTop * math.cos(data.anchor)
    y1 = y - anchorTop * math.sin(data.anchor)
    canvas.create_line(x0, y0, x1, y1, fill="red", width=2, dash=(2,4))

def findBottomRow(data) :
    Bottom = 0
    for row in range(len(data.bubbles)-1, 0, -1):
        for col in range(len(data.bubbles[row])) :
            if data.bubbles[row][col] != None : 
                return row
    return Bottom

# remove the lowest row that has 1 or more bubbles
def removeLine(data) :
    removalRow = findBottomRow(data)
    for col in range(len(data.bubbles[removalRow])) :
        data.bubbles[removalRow][col] = None       
    ...
    if not data.gameOver :
        ...
        if event.keysym == "P" or event.keysym == "p" :
            ...
            if len(data.items) > 0 :
                ...
                if itemType == "attack" :
                    signal = item
                elif item == "shield" or item =="deflect" :
                    signal = item
...
class Bubble(object) :
    r = 17 # bubble size
    def __init__(self, row, col, x, y, color, feature=None, Bubbletype=None) :
        self.row = row
        self.col = col
        self.x = x
        self.y = y
...
class tempBubble(Bubble) :
    def __init__(self, row, col, x, y, color, feature=None, Bubbletype=None) :
        super().__init__(row, col, x, y, color, feature, Bubbletype)
        self.row = -2
        self.col = -2
        self.dx = 0
        self.dy = 0
...
class Item(tempBubble) :
    def __init__(self, row, col, x, y, color, feature, Bubbletype) :
        super().__init__(row, col, x, y, color, feature, Bubbletype)
        self.feature = feature
        self.type = Bubbletype # attack or help
